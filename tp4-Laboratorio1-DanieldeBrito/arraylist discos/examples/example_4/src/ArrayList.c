 ...
 * \param pList ArrayList* Pointer to arrayList
 * \return int Return (-1) if Error [pList is NULL pointer] - (0) is ok
 *
 */
...
/** \brief  Indicate lenght of array
 * \param pList ArrayList* Pointer to arrayList
 ...
 * \param index int Index of the element
 * \param pElement void* Pointer to element
 * return returnAux; \return int Return (-1) if Error [pList or pElement are NULL pointer or invalid index]
 *                  - ( 0) if Ok
 *
 */
 ...
 * \param pList ArrayList* Pointer to arrayList
 * \param index int Index of the element
 * return returnAux;\return int Return (-1) if Error [pList is NULL pointer or invalid index]
 *                  - ( 0) if Ok
 */
        ...
        for(i=0; i<(al_len(pList))-1; i++)
        {
            for(j=i+1; j<(al_len(pList)); j++)
            {
                if(pFunc(al_get(pList,i),al_get(pList,j))==-1 && order==0)
                {
                    ...
                    //pAuxiliar=al_pop(pList,i);
                    //al_push(pList,i,pAuxiliar);
...
int resizeUp(ArrayList* pList)
{
    ...
    if(pList!=NULL)
    {
        pAux=realloc(pList->pElements,(sizeof(void*)*(pList->reservedSize+AL_INITIAL_VALUE)));
        if(pAux!=NULL)
        {
            pList->pElements=pAux;
            pList->reservedSize=pList->reservedSize+AL_INITIAL_VALUE;
            returnAux=0;
        }
    }
    return returnAux;
